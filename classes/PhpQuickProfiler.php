...
class PhpQuickProfiler {
	...
	public function __construct($startTime, $config = '/pqp/') {
		...
		//add_filter('query',array(&$this,'filterQueryData'));
	}
	...
	----------------------------------------------------------*/
	public function filterQueryData($query) {
		$this->db->query($query);
		return $query;
	}
	...
	public function gatherQueryData() {
		...
		$temp = array_unique($temp2);
		$queryTotals['dupes'] = count($temp2) - count($temp);

		$queryTotals['size'] = number_format($queryTotals['size']);
